...
const _0x4f2d = async () => {
	const res = await fetch("/template/credit.json");
...
const Hero = () => {
	...
	useEffect(() => {
		...
		_0x4f2d().then((text) => setCredit(text));
	...
	return (
		<div className="flex justify-center flex-col h-screen -mt-20">
			...
			<div
				className={`absolute bottom-4 w-full text-center text-base md:text-lg font-medium text-white hover:text-gray-800 transition-colors ${
					isVisible ? "opacity-100 blur-none" : "opacity-0 blur-lg"
				}`}
			>
				{credit}
